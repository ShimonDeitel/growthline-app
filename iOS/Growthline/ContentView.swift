...
struct EntryFormView: View {
    ...
    @State private var stage: String = MeasurementEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(MeasurementEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Measurement" : "Edit Measurement")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = MeasurementEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
